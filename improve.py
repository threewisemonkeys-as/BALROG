...
We have played the game using these beliefs and perception. Here is the episode outcome and trajectory:
...
- Belief analysis: What beliefs were missing, incorrect, or correctly applied?
- Perception analysis: What information was helpful, what information was misleading / incorrect and what missing information could have helped if extracted by the perception module?
...
async def _get_hypothesis_summary_async(
    config: DictConfig,
    hypothesis: str,
    outcome_header: str,
    traj_text: str,
    ...
    """Get a summary focused on hypothesis evaluation (async).

    Args:
        config: Configuration containing model information
        hypothesis: The hypothesis that was being tested in this episode
        outcome_header: Formatted episode outcome header
        traj_text: Trajectory text content
        trajectory_path: Path to the trajectory file (for logging)

    Returns:
        Summary text focused on hypothesis evaluation
    """
    prompt = f"""We are playing a game and testing a specific hypothesis how it works.

We were testing the following hypothesis in this episode:
=== HYPOTHESIS ===
{hypothesis}
=== END HYPOTHESIS ===
...
Analyse whether the above trajectory provides enough evidence to validate or invalidate the given hypothesis.

Format your response in XML style as -
<think>
Analyse whether we can validate or invalidate the hypothesis from the given trajectory.
</think>
<summary>
A summary of the reasons behind whether the given hypothesis is correct or not, or an explanation of insufficient evidence for a conclusion.
</summary>
"""
    ...
    # Call LLM asynchronously
    logging.info(f"Calling LLM to summarize hypothesis for: {trajectory_path}")
    if config.client.client_name == "vllm":
        model_name = f"hosted_vllm/{config.client.model_id}"
    else:
        model_name = f"{config.client.client_name}/{config.client.model_id}"
...
async def get_episode_summary_async(
    ...
    2. Hypothesis evaluation (if hypothesis is provided)
    ...
    Args:
        ...
        hypothesis: Optional hypothesis that was being tested in this episode
    ...
    instructions_perception_summary = await _get_instructions_perception_summary_async(
        config, instruction, perception, outcome_header, traj_text, trajectory_path
    )

    # If hypothesis is provided, make a separate call for hypothesis summary
    if hypothesis:
        hypothesis_summary = await _get_hypothesis_summary_async(
            config, hypothesis, outcome_header, traj_text, trajectory_path
        )
...
{hypothesis_summary}"""
        ...
        return instructions_perception_summary
